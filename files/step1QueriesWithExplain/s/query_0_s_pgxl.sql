...
,store
,customer
,(select ctr_store_sk, avg(ctr_total_return) as avg_total_return
from customer_total_return
group by ctr_store_sk) avg_ctr2
where ctr1.ctr_total_return > avg_total_return*1.2
and ctr1.ctr_store_sk = avg_ctr2.ctr_store_sk
...
explain analyze select  a.ca_state state, count(*) cnt
 from customer_address a
     ...
     ,date_dim d
     ,item i
	,(select i_category, avg(i_current_price) as avg_current_price
 	     from item 
 	     group by i_category) avg_icp
 	...
 	and d.d_month_seq = 
 	     (select distinct (d_month_seq)
 	      from date_dim
               where d_year = 2002
 	        and d_moy = 3 )
 	and i.i_current_price > 1.2 * avg_current_price 
 	and avg_icp.i_category = i.i_category
 group by a.ca_state
 having count(*) >= 10
